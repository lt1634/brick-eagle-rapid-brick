    ...
    <article className="overflow-hidden rounded-xl border border-border bg-raised shadow-card">
      <div className="relative h-40 overflow-hidden bg-surface-2">
        {listing.image ? (
          ...
          <div className="flex size-full items-end bg-[radial-gradient(80%_60%_at_20%_0%,color-mix(in_oklab,var(--color-primary)_18%,transparent),transparent_55%),linear-gradient(180deg,#d7e4dc,var(--color-surface-2))] px-4 py-3">
            <p className="font-display text-lg text-primary">{listing.estate || "梅窩"}</p>
          </div>
        )}
        <div className="absolute left-3 top-3 flex gap-1.5">
          ...
          {listing.kind === "village" ? <Badge variant="outline">村屋</Badge> : null}
        </div>
      </div>
